...
open-source-development-backlog:
	python3 scripts/release/open-source-development-backlog.py $(if $(LOCAL_ONLY),--local-only) $(if $(PHASE),--phase "$(PHASE)") $(if $(RELEASE_GATES_JSON),--release-gates "$(RELEASE_GATES_JSON)") $(if $(CUSTOMER_RELEASE_GATE),--customer-release-gate "$(CUSTOMER_RELEASE_GATE)") $(if $(VISUAL_REGRESSION_GATE),--visual-regression-gate "$(VISUAL_REGRESSION_GATE)") $(if $(TRACE_SVG_STRESS),--trace-svg-stress "$(TRACE_SVG_STRESS)") $(if $(MODEL_LIFECYCLE_GATE),--model-lifecycle-gate "$(MODEL_LIFECYCLE_GATE)") $(if $(CUSTOMER_ENV_CERTIFICATION_GATE),--customer-env-certification-gate "$(CUSTOMER_ENV_CERTIFICATION_GATE)") $(if $(PLUGIN_CATALOG_GATE),--plugin-catalog-gate "$(PLUGIN_CATALOG_GATE)") $(if $(ONBOARDING_MANIFEST),--onboarding-manifest "$(ONBOARDING_MANIFEST)") --out-json "$(or $(OUT_DIR),build/open-source-readiness)/open-source-development-backlog.json" --out-md "$(or $(OUT_DIR),build/open-source-readiness)/open-source-development-backlog.md"

open-source-milestone:
	python3 scripts/release/open-source-milestone.py --open-source-readiness-gate "$(or $(OPEN_SOURCE_READINESS_GATE),build/open-source-readiness/open-source-readiness-gate.json)" --open-source-readiness-backlog "$(or $(OPEN_SOURCE_READINESS_BACKLOG),build/open-source-readiness/open-source-readiness-backlog.json)" --open-source-development-backlog "$(or $(OPEN_SOURCE_DEVELOPMENT_BACKLOG),build/open-source-readiness/open-source-development-backlog.json)" --release-gates "$(or $(RELEASE_GATES_JSON),build/release-gate-status.json)" --release-evidence-consistency-gate "$(or $(RELEASE_EVIDENCE_CONSISTENCY_GATE),build/release-evidence/release-evidence-consistency-gate.json)" --model-lifecycle-gate "$(or $(MODEL_LIFECYCLE_GATE),build/evaluation/model-lifecycle-gate.json)" --visual-regression-snapshots "$(or $(VISUAL_REGRESSION_SNAPSHOTS),build/visual-regression/visual-regression-snapshots.json)" $(if $(ALLOW_MISSING),--allow-missing) --out-json "$(or $(OUT_DIR),build/open-source-readiness)/open-source-milestone.json" --out-md "$(or $(OUT_DIR),build/open-source-readiness)/open-source-milestone.md"
