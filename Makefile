...
soak-sample:
	...
	@python3 scripts/ops/collect-soak-sample.py $(if $(STATUS_URL),--status-url "$(STATUS_URL)") $(if $(STATUS_JSON),--status-json "$(STATUS_JSON)") $(if $(SOAK_HOST),--host "$(SOAK_HOST)") $(if $(SOAK_STARTED_AT_EPOCH),--started-at-epoch "$(SOAK_STARTED_AT_EPOCH)") --out "$(or $(OUT),build/performance/soak-samples.json)"
