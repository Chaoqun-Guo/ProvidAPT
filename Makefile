...
soak-sample:
	@if [ -z "$(STATUS_URL)" ] && [ -z "$(STATUS_JSON)" ]; then echo 'usage: make soak-sample STATUS_URL=http://localhost:18080/api/v1/status [SOAK_SAMPLE_COUNT=6] [SOAK_SAMPLE_INTERVAL_SECONDS=10] [SOAK_STARTED_AT_EPOCH=...] [OUT=build/performance/soak-samples.json]'; exit 2; fi
	@python3 scripts/ops/collect-soak-sample.py $(if $(STATUS_URL),--status-url "$(STATUS_URL)") $(if $(STATUS_JSON),--status-json "$(STATUS_JSON)") $(if $(SOAK_HOST),--host "$(SOAK_HOST)") $(if $(SOAK_STARTED_AT_EPOCH),--started-at-epoch "$(SOAK_STARTED_AT_EPOCH)") --count "$(or $(SOAK_SAMPLE_COUNT),1)" --interval-seconds "$(or $(SOAK_SAMPLE_INTERVAL_SECONDS),0)" --out "$(or $(OUT),build/performance/soak-samples.json)"
...
capture-quality-trend:
	@if [ -z "$(GATE_REPORTS)" ]; then echo 'usage: make capture-quality-trend GATE_REPORTS="build/capture-quality/run-a.json build/capture-quality/run-b.json" [OUT_DIR=build/capture-quality]'; exit 2; fi
	python3 scripts/ops/capture-quality-trend.py $(foreach report,$(GATE_REPORTS),--gate-report "$(report)") --out-json "$(or $(OUT_DIR),build/capture-quality)/capture-quality-trend.json" --out-md "$(or $(OUT_DIR),build/capture-quality)/capture-quality-trend.md"
