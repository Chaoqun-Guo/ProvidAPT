...
observability-pack-check:
	python3 scripts/ops/observability-pack-check.py --prometheus "$(or $(PROMETHEUS_CONFIG),scripts/docker/prometheus.yml)" --alerts "$(or $(PROMETHEUS_ALERTS),scripts/docker/providapt_alerts.yml)" --dashboard "$(or $(GRAFANA_DASHBOARD),scripts/docker/providapt_dashboard.json)" $(if $(PROVIDAPT_SERVER_URL),--server "$(PROVIDAPT_SERVER_URL)") $(if $(PROVIDAPT_API_KEY),--api-key "$(PROVIDAPT_API_KEY)") --out-json "$(or $(OUT_DIR),build/observability)/observability-pack-check.json" --out-md "$(or $(OUT_DIR),build/observability)/observability-pack-check.md"
