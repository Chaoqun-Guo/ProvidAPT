...
`v1.2.3-rc.1` is a commercial release candidate focused on release evidence, vulnerability scanning, package smoke testing, production configuration readiness, and customer handoff material.
...
- Monitoring bundle validation for commercial release readiness.
- Host-mode package smoke testing for Debian, RPM, and tarball artifacts.

## Changed

- Commercial release checks now require archive, Debian, RPM, Helm, and monitoring artifacts by default.
...
- Added `govulncheck` reachable-code scanning to the commercial release pipeline.
...
- `providaptctl -release-check`: commercial ready with 16 passed checks, 0 warnings, 0 waived, and 0 failed.
