...
The following kernel options must be enabled (check with `scripts/kernel_probe.sh`):
...
sudo cp scripts/providapt-cgroup.service /etc/systemd/system/providapt.service
...
The service unit (`scripts/providapt-cgroup.service`) includes:
...
sudo bash scripts/setup_cgroup.sh --remove
...
│   ├── bpf/
│   │   ├── lsm_hooks.bpf.c   # Core LSM hooks
│   │   ├── defense.bpf.c     # Self-defence
│   │   ├── memory.bpf.c      # Memory attack detection
│   │   └── network.bpf.c     # Enhanced network events
│   └── include/
│       ├── providapt.h        # Shared definitions
│       └── taint.h           # Taint flags
├── userspace/                 # Go userspace programs
│   ├── cmd/                   # CLI binaries
