...
Every event in the ring buffer is a fixed-size `struct event` (332 bytes), defined in `kernel/include/providapt.h`:
...
**Algorithm** (in `pkg/compact/reducer.go`):
...
**Algorithm** (in `pkg/compact/summary.go`):
