...
func TestGraphAddFileWrite(t *testing.T) {
	...
	edges := g.Edges()
	// Versioning creates wasDerivedFrom (v2→v1) + wasGeneratedBy (v2→proc) = 2 edges
	if len(edges) != 2 {
		t.Fatalf("expected 2 edges (wasDerivedFrom + wasGeneratedBy), got %d", len(edges))
	}
	// One edge should be wasGeneratedBy
	found := false
	for _, e := range edges {
	...
	if !found {
		t.Error("expected wasGeneratedBy edge")
	}
}
...
func TestGraphDeduplicateNodes(t *testing.T) {
	...
	// Same process opens two files — manually set different inodes so files
	// don't collide (makeEvent generates inode = pid * 1000).
	e1 := makeEvent(syscall.EventFileOpen, 100, 1, 1000, "cat", "/etc/hosts")
	e1.Inode = 100001
	g.AddEvent(e1)

	e2 := makeEvent(syscall.EventFileOpen, 100, 1, 1000, "cat", "/etc/resolv.conf")
	e2.Inode = 100002
	g.AddEvent(e2)

	stats := g.Stats()
	if stats.Nodes != 3 { // 1 process + 2 files (versioning creates v1 for each)
...
func TestGraphMultipleTypes(t *testing.T) {
	...
	// NOTE: makeEvent generates inode = pid*1000, so we manually set different
	// inodes for different files to avoid node collisions.

	// 1. bash forks curl
	g.AddEvent(makeForkEvent(100, 200, 1000, "bash"))
	// 2. curl connects (exec event with binary path) — inode = 200*1000 = 200000
	g.AddEvent(makeEvent(syscall.EventProcessExec, 200, 100, 1000, "curl", "/usr/bin/curl"))
	// 3. curl reads a sensitive file — needs unique inode
	e3 := makeEvent(syscall.EventFileOpen, 200, 100, 1000, "curl", "/etc/shadow")
	e3.Inode = 200001
	g.AddEvent(e3)
	// 4. curl writes exfil data — needs unique inode
	e4 := makeWriteEvent(syscall.EventFileCreate, 200, 1000, "curl", "/tmp/exfil.dat")
	e4.Inode = 200002
	g.AddEvent(e4)

	stats := g.Stats()
	// Nodes: p:100 (bash), p:200 (curl), f:200000:8:3#v1 (curl binary),
	//   f:200001:8:3#v1 (shadow), f:200002:8:3#v1 (exfil.prev),
	//   f:200002:8:3#v2 (exfil.new) = 6
	// (FileCreate versioning creates a prev+v1 and new+v2 node)
	if stats.Nodes != 6 {
		t.Errorf("expected 6 nodes, got %d", stats.Nodes)
	}
	// Edges: wasInformedBy(child→parent), used(curl→binary),
	//   used(curl→shadow), wasDerivedFrom(exfil_v2→v1),
	//   wasGeneratedBy(exfil→curl) = 5
	if stats.Edges != 5 {
