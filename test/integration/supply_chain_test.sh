#!/usr/bin/env bash
# ProvidAPT Supply Chain Attack -Ultimate Integration Test
#
...
header "ProvidAPT  - Supply Chain Attack Integration Test"
...
log "Simulating python3 executing evil_runner.py  - memfd_create  - mprotect RW - X..."
...
NODE_RX=$(create_node "memory" "rx:0x7f1234:$((ATTACK_PID + 1))" "rw - x @0x7f1234" "${ATTACK_HOST}" "${ATTACK_AGENT}" \
...
create_edge "p:$((ATTACK_PID + 1))" "f:evil_runner.py:${ATTACK_PID}" "prov:used" "${ATTACK_HOST}" \
    '{"host_id":"'"${ATTACK_HOST}"'","exec_chain":"pip_install - emfd_exec"}'
...
if [ "${GRAPH_COUNT}" -ge 5 ]; then
    pass "Graph has ${GRAPH_COUNT} nodes ( -  required)"
else
    fail "Graph only has ${GRAPH_COUNT} nodes (expected  - )"
fi
...
if [ "${HOST_A_ENTRIES}" -ge 3 ]; then
    pass "Host-a has ${HOST_A_ENTRIES} indexed entries ( - : pip, python3, network)"
else
...
pass "Memory forensics auto-triggered on mprotect RW - X"
...
if [ "${BLAST_RESULT}" -ge 2 ]; then
    ...
    fail "Blast radius only covers ${BLAST_RESULT} hosts (expected  - )"
fi
...
echo "  Phase 2  - Fileless:     memfd_create + mprotect RW - X"
