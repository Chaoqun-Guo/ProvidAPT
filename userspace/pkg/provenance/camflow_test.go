...
func makeWrite(pid, uid uint32, comm, path string) *collector.Event {
	e := makeEvent(syscall.EventFileModify, pid, uid, comm, path)
	e.FFlags = 1
	return e
}

func makeCreate(pid, uid uint32, comm, path string) *collector.Event {
	e := makeEvent(syscall.EventFileCreate, pid, uid, comm, path)
...
func TestGraphReadTargetsLatest(t *testing.T) {
	...
	// Write creates v1, then read should use v1
	g.AddEvent(makeCreate(100, 0, "vi", "/tmp/data.txt"))
	// Read event
	evt := makeEvent(syscall.EventFileOpen, 200, 1000, "cat", "/tmp/data.txt")
	evt.Inode = 100000 // same inode as the write
	g.AddEvent(evt)

	// Verify used edge points to v1
	edges := g.Edges()
	readEdge := false
	for _, e := range edges {
		if e.Relation == ProvUsed && e.Source == "p:200" {
			if e.Target == "f:100000:8:3#v1" {
				readEdge = true
			}
		}
	}
	if !readEdge {
		t.Error("expected used edge to target version v1")
	}
}
...
func TestCredTrackerSetuidCreatesNode(t *testing.T) {
	...
	credID := ct.OnExec(evt, ts, g)
	if credID == "" {
		t.Fatal("expected credential node for setuid exec")
	}

	// Verify credential node exists
	if _, ok := g.LookupNode(credID); !ok {
		t.Errorf("credential node %s not found in graph", credID)
	}
}
...
func TestPrunerKeepsInteresting(t *testing.T) {
	...
	g.AddEvent(makeEvent(syscall.EventFileOpen, 100, 0, "cat", "/etc/shadow"))
	// Normal activity (should be pruned)
	g.AddEvent(makeEvent(syscall.EventFileOpen, 200, 0, "cat", "/tmp/trivial.txt"))
...
func TestPrunerNetworkInteresting(t *testing.T) {
	...
	g.AddEvent(makeEvent(syscall.EventFileOpen, 100, 0, "sshd", "/etc/hosts"))
...
func TestPrunerExtraSet(t *testing.T) {
	g := NewGraph()
	g.AddEvent(makeEvent(syscall.EventFileOpen, 100, 0, "bash", "/tmp/foo.txt"))
	g.AddEvent(makeEvent(syscall.EventFileOpen, 200, 0, "bash", "/tmp/bar.txt"))
...
func TestPruneMultipleCalls(t *testing.T) {
	...
	g.AddEvent(makeEvent(syscall.EventFileOpen, 100, 0, "bash", "/etc/shadow"))
	g.AddEvent(makeEvent(syscall.EventFileOpen, 200, 0, "bash", "/tmp/trivial.txt"))
